...
import { buyObjOld } from "../../api/gameFunctionsOld";
...
export default function ShopList(props) {
    ...
    const buyObjFunc = (obj, i) => {
        if (props.state.gameStates.gold > (counterArray[i] * obj.gold)) {
            buyObjOld(obj.name, counterArray[i])
        }
    }
                            ...
                            <tbody>
                                {mappedItems.map((el, i) => {
                                    return <tr key={el.name} id={el.name} className={props.state.gameStates.gold < (counterArray[i] * el.gold) ? 'disabled' : ''} onClick={openDetailsFunc}>
                                        <td>{props.values ? props.state.gameStates.shop[Object.keys(props.state.gameStates.shop).find(x => props.state.gameStates.shop[x].name === el.name)].count :'∞'}</td>
                                        <td>
